...
def test_auto_create_chantier_from_devis(db_session):
    ...
    print(f"✅ Chantier auto-créé: {chantier.type}, début {chantier.date_debut}")
...
def test_chantier_date_debut_logic(db_session):
    ...
    assert chantier.date_debut > datetime.now().date()

    # Devrait être dans un délai raisonnable (moins de 30 jours)
    days_until_start = (chantier.date_debut - datetime.now().date()).days
...
def test_check_planning_conflicts(db_session):
    ...
    existing = Chantier(
        client_id=client.id,
        titre="Test chantier existant",
        date_debut=(datetime.now() + timedelta(days=7)).date(),
        date_fin=(datetime.now() + timedelta(days=8)).date(),
        status="planifie",
        type="bureaux"
    )
...
def test_notify_client_chantier(db_session):
    ...
    chantier = Chantier(
        ...
        date_debut=(datetime.now() + timedelta(days=3)).date(),
        status="planifie",
        type="copropriete"
    )
    ...
    # Tester notification
    result = notify_client_chantier(db_session, chantier)

    # Devrait retourner True/False
    assert isinstance(result, bool)
    print(f"✅ Notification client: {'success' if result else 'failed'}")
